...
new Vue({
    ...
    created: function () {
        //if server emits 'user joined', update connectedUser[]
        ...
        socket.on('chat.message', function (message) {
            time2 = new Date(message.timestamp)
            message.timestamp = time2.toLocaleTimeString()
            this.messages.push(message)
        }.bind(this));


        //if server emits 'user typing'
        socket.on('user typing', function (username) {
            this.areTyping.push(username)
        }.bind(this));

        //if server emits 'stopped typing'
        ...
        //if server broadcasts 'user left', remove leaving user from connectedUser[]
